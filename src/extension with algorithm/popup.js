...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize the active tab to 'settings' and load its content
  switchPopupTab('settings');
});
...
function switchPopupTab(tabName) {
  ...
  const selectedButton = document.querySelector(`[onclick="switchPopupTab('${tabName}')"]`);
  if (selectedButton) {
    selectedButton.classList.add('active');
  }

  // If profile tab, load summary
  if (tabName === 'profile') {
    loadProfileSummary();
  }
}
...
/**
 * Open profile setup in new tab/window
 */
...
async function loadProfileSummary() {
  try {
    ...
    const profile = result['jh_user_profile'];

    const statusEl = document.getElementById('profile-status-text');

    if (!profile) {
      statusEl.innerHTML = '<span style="color: #f39c12;">⚠️ Profile not configured</span>';
      return;
    }

    const salary = profile.preferences?.salary_floor || 0;
    const remote = profile.preferences?.remote_requirement || 'unknown';
    const skills = (profile.background?.core_skills || []).length;
    const roles = (profile.background?.target_roles || []).length;

    const summary = `
      <span style="color: #2ecc71;">✓ Profile Configured</span><br>
      <small>
        Salary Floor: $${salary.toLocaleString()} | 
        Remote: ${remote.replace(/_/g, ' ')} |
        Skills: ${skills} | 
        Target Roles: ${roles}
      </small>
    `;
    ...
    console.error('[Popup] Error loading profile summary:', error);
    document.getElementById('profile-status-text').textContent = 'Error loading profile';
  }
}
